import { AppRoot, el } from "@commonmodule/app";
...
export default class SigorSparrowEditForm extends NFTEditForm {
    ...
    constructor(nftData) {
        ...
        this.prepend(this.main = el("main", this.playerContainer = el(".player-container", this.nftPlayer = new SigorSparrowPlayer(nftData)), this.dialogueInput = new Input({
            label: "Dialogue",
            value: nftData.traits?.["Dialogue"] || "",
            placeholder: "Enter dialogue here...",
            debounceDelay: 500,
        })));
        this.attributeEditor.on("dataChanged", () => this.nftPlayer.setData(this.getData()));
        this.dialogueInput.on("valueChanged", () => this.nftPlayer.setData(this.getData()));
